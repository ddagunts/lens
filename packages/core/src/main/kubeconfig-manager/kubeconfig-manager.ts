...
export class KubeconfigManager {
  ...
  async ensureOriginalKubeconfigPath(): Promise<string> {
    if (this.originalTempFilePath === null || !(await this.dependencies.pathExists(this.originalTempFilePath))) {
      return await this.ensureOriginalKubeconfigFile();
    }

    return this.originalTempFilePath;
  }
  ...
  protected async ensureOriginalKubeconfigFile() {
    try {
      await this.dependencies.kubeAuthProxyServer.ensureRunning(); // not sure if we need to wait for this, but alas
      return this.originalTempFilePath = await this.createOriginalKubeconfig();
    } catch (error) {
      throw new Error(`Failed to create temp original config for auth-proxy: ${error}`);
    }
  }
